...
def comment_create_question(request, question_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            comment = form.save(commit=False)  # 임시저장 이 아닌 바로 저장
            comment.author = request.user
            comment.create_date = timezone.now()
            comment.question = question
            comment.save()
            return redirect(
                '{}comment_{}'.format(resolve_url('pybo:detail', question_id=comment.question_id), comment.id))
    else:
        form = CommentForm()  # 무슨 코드지?
    context = {'form': form}
